...
- **Агент (авто-мод)** — автономний аналіз системи: знімок → ШІ → висновок + самостійно створені нові правила в БЗ
- **Чат** — плаваюче вікно на всіх сторінках: розмова з агентом; агент вміє і редагувати дані (валідовані JSON-команди insert/update/delete)
- **Тестування** — самотест із 7 перевірок: БД, читання всіх таблиць, CRUD-цикл, розкладка, шрифт PDF, ключ/endpoint ШІ, відповідь моделі
...
- **API ключ Groq** — `gsk_…`
- **Base URL** — порожньо = дефолт Groq `https://api.groq.com/openai/v1`
- **Модель** — актуальну production-модель дивись у [доках Groq](https://console.groq.com/docs/models) (наприклад `openai/gpt-oss-120b`)
...
POST   /api/chat          чат з агентом (уміє редагувати БД)
POST   /api/agent/analyze авто-мод: аналіз → нові правила в БЗ
POST   /api/selftest      самотест системи (7 перевірок)
GET/PUT /api/config
```
